...
gains = [.1 .1 .1 .1 .1]; % can change this to be varying
...
%% Test parameters
p = 10;
mu = linspace(.0001,.1,500);
mse_lms = zeros(1,length(mu));
mse_nlms = zeros(1,length(mu));
for i = 1:length(mu)
    xc_lms = perform_lms(xn,ref_noise,mu(i),p);
    xc_nlms = perform_nlms(xn,ref_noise,mu(i),p);
...
best_mu_lms = mu(loc_lms)
[min_mse_nlms,loc_nlms] = min(mse_nlms);
best_mu_nlms = mu(loc_nlms)

lambda = linspace(.98,1,10);
mse_rls = zeros(1,length(lambda));
for i = 1:length(lambda)
    xc_rls = perform_rls(xn,ref_noise,lambda(i),1,p);
...
best_lam_rls = lambda(loc_rls)

gamma = linspace(.5,1,100);
mse_afa = zeros(1,length(gamma));
for i = 1:length(gamma)
    xc_afa = perform_afa(xn,ref_noise,gamma(i),p);
